...
export default function Clusters(){
    ...
    const dataClusters = async() =>{
        ...
        getClusters.data.forEach((cluster, index) =>{
            ...
            dataTable[index] = [(index + 1),
                                cluster.nombre,
                                cluster.foto,
                                (cluster.state === 1) ? cluster.state='Habilitado':cluster.state='Inhabilitado',
                                userNames,
                                cluster.createdAt,
                                [cluster._id,
                                    cluster.nombre,
    ...
    $(function(){
        ...
        tablaCluster.on("order.dt search.dt", function(){
            tablaCluster.column(0, {"search":"applied", "order":"applied"}).nodes().each(function(cell, i){
                cell.innerHTML = i+1
            })
        }).draw()
    })
                    ...
                    <div className="container-fluid">
                        ...
                        <div className="col-lg-12">
                                    <div className="card card-primary">
                                        <div style={{backgroundColor:"#FF9B00"}} className="card-header">
                                            <h5 className="m-0">
                                                <button className="btn btn-light" data-toggle="modal" data-target="#addCluster"> Crear Nuevo Cluster</button>
                                            </h5>
                                        </div>
